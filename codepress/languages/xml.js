 ...
 * By NeoNexus DeMortis
 */

Language.syntax = [
	{
		input : /(&lt;[\w]+.*?[\/]?&gt;)/g,
		output : '<tag>$1</tag>' // all tags
	},{
		input : /([\w]*?)=(".*?")/g,
		output : '<attribname>$1</attribname>=<attrib>$2</attrib>'
	},{
		input : /(&lt;\/.*?&gt;)/g,
		output : '<tag>$1</tag>' // closing tags
	},{
		input : /(&lt;!--.*?--&gt;)/g,
		output : '<comm>$1</comm>' // comments
	},{
		input : /(&lt;!\[CDATA\[(?:.*?)\]\]&gt;)/g,
		output : '<cdata>$1</cdata>' // comments
	},{
